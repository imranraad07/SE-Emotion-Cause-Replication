...
def get_model_path(model_name):
    ...
    if model_name not in model_paths:
        raise ValueError("Invalid model_name. Choose from bertand roberta.")
...
def train_model(model, train_dataloader, epochs, delta, optimizer, scheduler, val_dataloader, output_file, col):
    model.to(device)
    f1 = -0.1
    loss_val = 100
...
def parse_arguments():
    ...
    parser.add_argument("--model_name", type=str, default='bert', required=True, choices=["bert", "roberta"], help="Flag. Choose from bert and roberta.")
...
def main():
    ...
    train_model(model, train_dataloader, epochs, delta, optimizer, scheduler, val_dataloader, output_file, col)
